...
const useSaveFeeds = (
    ...
    React.useEffect(() => {
        if (responseArray)
            setRawRssArray((oldArray: string[]) =>
                oldArray.slice(oldArray.length).concat(responseArray)
            );
    }, [responseArray]);
    ...
    React.useEffect(() => {
        if (rawRssArray) {
            ...
            const parseResult = rawRssArray.map(
                (rawRss: string, index: number) => {
                    const id = totalFeeds[index]
                        ? totalFeeds[index].feeds.length
                        : 0;
                    ...
                    if (
                        !areEqual(feedOriginName, totalFeeds[index]?.originName)
                    ) {
                        const result = {
                            id: originId,
    ...
    if (isFilterFavorite) {
        return newFeeds.map((feedData: ParseResultType) => {
            const filteredFeeds = feedData.feeds?.filter(
